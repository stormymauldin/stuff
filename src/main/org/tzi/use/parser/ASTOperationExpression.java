...
class ASTOperationExpression extends ASTExpression {
    private MyToken fOp;
	...
	case SRC_OBJECT_TYPE + DOT + NO_PARENTHESES:
		...
		if ( dstEnd != null )
		    res = genNavigation(fOp, srcClass, srcExpr, dstEnd);
		else {
		    // (1) predefined OCL operation
		    res = genStdOperation(ctx, fOp, opname, fArgExprs);
		}
...
//  	    throw new SemanticException(fOp,
//  		"If you want to apply an operation to a collection, please use an `->' instead of a `.'. If you are trying to apply the shorthand notation for `collect' - sorry, this is not yet supported. Please use the explicit notation.");
