...
/**
 * Set value. The set is read-only outside of this package. Changes
 * always result in a new Set. All methods operating on element values
 * can throw an IllegalArgumentException if a passed value's type does
 * not conform to the element type of the set.
 *
...
public class SetValue extends CollectionValue {
    ...
    /**
     * Constructs a set and adds all values. Elements are type checked
     * as they get inserted into the set.
     *
...
//      }
	...
	// copy this set
	SetValue res = new SetValue(elemType(), fElements);
	res.fElements.remove(v);
	return res;
    }

    /** 
     * Return a new set which is the symmetric difference of this and
     * v, ie. (A - B) U (B - A).
     *
     * @pre T2 <= T1, if this has type Set(T1) and v has type Set(T2).  
     */
    ...
    /**
     * Returns a new "flattened" set. This set must have collection
     * elements.  */
    ...
    /** 
     * Two sets are equal iff they contain the same elements. However,
     * the declared types may be different if the second is a
     * subtype of the first.
     *
     * @pre T2 <= T1, if this has type Set(T1) and 
     *      obj has type Set(T2).  
     */
