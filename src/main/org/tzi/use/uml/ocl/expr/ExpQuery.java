...
/** 
 * Abstract base class for select, reject, collect, exists, forAll
 * and iterate expressions.
 *
 * @version 	$ProjectVersion: 2-1-0-release.1 $
 * @author 	Mark Richters
 */

abstract class ExpQuery extends Expression {
    
    /** 
     * List of element variables that will be bound to each range
    ...
    protected ExpQuery(Type resultType, 
		       VarDeclList elemVarDecls, 
		       Expression rangeExp, 
		       Expression queryExp) 
	...
	if ( ! fRangeExp.type().isCollection() )
	    throw new ExpInvalidException(
		   "Range expression must be of type " +
		   "`Collection', found `" +
		   fRangeExp.type() + "'.");
	...
	for (int i = 0; i < fElemVarDecls.size(); i++) {
		...
		throw new ExpInvalidException("Type `" + 
		      vd.type() + 
     ...
     * <code>moreElemVars</code> may be null if there is at most one
	...
	while ( res != doExists && collIter.hasNext() ) {
		...
		// eventually
		res = evalExistsOrForAll0(nesting + 1, rangeVal, ctx, doExists);
	    } else {
		...
		// stop iteration when expression is true (exists) or
		// false (forAll)
		if ( ((BooleanValue) queryVal).value() == doExists )
		    res = doExists;
	    }
    ...
    /** 
     * Return name of concrete query expression,
     * e.g. `select'. Defined by subclasses.  
     */
